...
// projects/client/src/app/core/services/fund-list.service.ts
var _FundListService = class _FundListService {
  constructor(fundService) {
    this.fundService = fundService;
    this.$mutualFunds = new BehaviorSubject([]);
    this.destroy$ = new Subject();
    this.BEST_LIMIT_INTERVAL = 3e4;
  ...
  // }
  fetchAllMutualFunds() {
    this.fundService.getAllMutualFundsDetail().pipe(switchMap((funds) => {
      const fundsWithDetails$ = funds.map((fund) => fund.investType === 2 ? this.getBestLimit(fund.mutualFundId).pipe(map((limit) => __spreadProps(__spreadValues({}, fund), {
        buyPrice: limit?.buyPrice,
        sellPrice: limit?.sellPrice
      }))) : of(fund));
      return forkJoin(fundsWithDetails$);
      ...
      return this.getNewFundAttachmentAll(fundIds).pipe(map((attachments) => {
        ...
        return fundsWithDetails.map((fund) => __spreadProps(__spreadValues({}, fund), {
  ...
  getBestLimit(fundId) {
    return this.fundService.bestLimitByFundId(fundId).pipe(map((result) => ({
      sellPrice: result?.sellPrice,
      buyPrice: result?.buyPrice
    })));
  }
  getFundAttachments(fund) {
    return this.fundService.getFundAttachments(fund.mutualFundId).pipe(map((res) => __spreadProps(__spreadValues({}, fund), {
      attachments: res?.result ?? []
    })));
  }
  getFundAttachmentAll(fundIds) {
    return this.fundService.getFundAttachmentAll(fundIds).pipe(map((res) => res?.result ?? []));
  }
  getNewFundAttachments(fund) {
    return this.fundService.getFundAttachments(fund.mutualFundId).pipe(map((res) => __spreadProps(__spreadValues({}, fund), {
      attachments: res?.result ?? []
    })));
  }
  getNewFundAttachmentAll(fundIds) {
    return this.fundService.getNewFundAttachmentAll(fundIds).pipe(map((res) => res?.result ?? []));
  }
  startUpdateInterval() {
    timer(this.BEST_LIMIT_INTERVAL, this.BEST_LIMIT_INTERVAL).pipe(takeUntil(this.destroy$)).subscribe(() => {
  ...
  updateBestLimit() {
    ...
    const updatedFunds$ = funds.map((fund) => this.getBestLimit(fund.mutualFundId).pipe(map((limit) => __spreadProps(__spreadValues({}, fund), {
      buyPrice: limit?.buyPrice,
      sellPrice: limit?.sellPrice
    }))));
  ...
  sendFakeDataUpdatesToTest() {
    ...
    const updatedFunds = currentFunds.map((fund) => {
      if (fund.fundType === 5) {
        return __spreadProps(__spreadValues({}, fund), {
          buyPrice: fund.buyPrice ? this.generateFakePrice(fund.buyPrice) : fund.buyPrice,
          sellPrice: fund.sellPrice ? this.generateFakePrice(fund.sellPrice) : fund.sellPrice,
    ...
    console.log("\u{1F504} FAKE DATA UPDATE - Timestamp:", (/* @__PURE__ */ new Date()).toLocaleTimeString(), "Updated fundType=5 count:", updatedCount);
  }
...
_FundListService.\u0275fac = function FundListService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _FundListService)(\u0275\u0275inject(FundService));
};
_FundListService.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _FundListService, factory: _FundListService.\u0275fac, providedIn: "root" });
var FundListService = _FundListService;
...
var _NgxEchartsDirective = class _NgxEchartsDirective {
  ...
  createChart() {
    ...
    return this.ngZone.runOutsideAngular(() => {
      ...
      return load().then(({
        init: init2
      }) => init2(dom, this.theme, this.initOpts));
    });
  }
  initChart() {
    return __async(this, null, function* () {
...
// projects/client/src/app/features/reports/requests-report/requests-filter/transactions.model.ts
var Filter = {
  reportFilter: {
    id: null,
    startDate: null,
...
//# sourceMappingURL=chunk-ZHG6NCW7.js.map
